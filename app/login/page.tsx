...
import { 
  ...
  ChevronRight, AlertCircle, CheckCircle2, ArrowLeft 
} from 'lucide-react';
...
function AuthContent() {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    if (authMode === 'signup') {
      ...
      if (error) setError(error.message);
      else { showToast('Check your email!', 'success'); setAuthMode('login'); }
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F8FAFC] p-4 font-sans text-slate-900 relative">
      ...
      <div className="max-w-md w-full bg-white rounded-[40px] shadow-2xl shadow-blue-900/5 p-8 md:p-12 border border-white relative overflow-hidden transition-all duration-300">
        ...
        <form onSubmit={authMode === 'forgot' ? handleResetPassword : handleAuth} className="space-y-5">
          ...
          {authMode === 'signup' && (
            <div className="relative group">
              ...
              <input type="text" placeholder="Full Name" className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none focus:bg-white focus:border-blue-500 text-sm font-medium transition-all" onChange={(e) => setName(e.target.value)} required />
            </div>
          )}
          ...
          <div className="relative group">
            ...
            <input type="email" placeholder="Email Address" className="w-full pl-12 pr-4 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none focus:bg-white focus:border-blue-500 text-sm font-medium transition-all" onChange={(e) => setEmail(e.target.value)} required />
          </div>
          ...
          {authMode !== 'forgot' && (
            <div className="relative group">
              ...
              <input type={showPassword ? "text" : "password"} placeholder="Password" className="w-full pl-12 pr-12 py-4 bg-slate-50 border border-slate-100 rounded-[20px] outline-none focus:bg-white focus:border-blue-500 text-sm font-medium transition-all" onChange={(e) => setPassword(e.target.value)} required />
          ...
          {authMode === 'login' && (
            <div className="flex justify-end">
              <button type="button" onClick={() => setAuthMode('forgot')} className="text-[10px] font-black text-blue-600 hover:text-blue-700 uppercase tracking-widest transition-colors">
                Forgot Password?
              </button>
            </div>
          )}

          <button disabled={loading} className="group w-full bg-slate-900 text-white py-4 rounded-[20px] font-bold text-lg hover:bg-blue-600 transition-all flex items-center justify-center gap-2 disabled:opacity-50 shadow-xl shadow-slate-200">
        ...
        <div className="mt-10 pt-8 border-t border-slate-100 text-center">
           {authMode === 'forgot' ? (
             <button onClick={() => setAuthMode('login')} className="flex items-center justify-center gap-2 w-full text-slate-400 hover:text-blue-600 text-sm font-bold transition-colors">
               <ArrowLeft size={16} /> Back to Sign In
             </button>
           ) : (
             <button onClick={() => { setAuthMode(authMode === 'login' ? 'signup' : 'login'); setError(''); }} className="text-blue-600 text-sm font-black hover:text-blue-700 underline underline-offset-8 decoration-2 transition-all">
